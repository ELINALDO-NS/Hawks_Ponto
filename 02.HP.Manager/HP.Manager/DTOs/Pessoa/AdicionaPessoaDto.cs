...
{
    public record AdicionaPessoaDto(int EmpresaId, int Matricula, string Nome, DateTime? DataNascimento, DateTime DataAdmissao, DateTime? DataDemissao, EnderecoDto? Endereco, string? Rg, string Cpf, string Pis, string? Telefone, string? TelefoneCelular, string? Email, bool ControlaPonto, DateTime? DataControlaPonto, DateTime? DataNaoControlaPonto, Sexo Sexo, float BaseHoras,AdicionaHorarioPessoaDto Horario, AdicionaCargoPessoaDto? Cargo, AdicionaEstruturaOrganizacionalPessoaDto EstruturaOrganizacional);
